...
func init() {
	...
	utilruntime.Must(smoothv1.AddToScheme(scheme))
...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "1b716e7a.smooth.k8s.io",
	...
	// Initialize context collection components
	setupLog.Info("Initializing context collection and LLM components")

	// 1. K8s Resource Collector
	collectorOptions := collector.DefaultCollectorOptions()
	clusterCollector := collector.NewCollector(mgr.GetClient(), collectorOptions)

	// 2. Prometheus Metrics Client (optional)
	prometheusOptions := metrics.DefaultPrometheusOptions()
	prometheusClient, err := metrics.NewPrometheusClient(prometheusOptions)
	if err != nil {
		setupLog.Error(err, "Failed to create Prometheus client")
		os.Exit(1)
	}
	if prometheusClient.IsEnabled() {
		setupLog.Info("Prometheus metrics collection enabled", "address", prometheusOptions.Address)
	} else {
		setupLog.Info("Prometheus metrics collection disabled (optional)")
	}

	// 3. Loki Log Client (optional)
	lokiOptions := logs.DefaultLokiOptions()
	lokiClient, err := logs.NewLokiClient(lokiOptions)
	if err != nil {
		setupLog.Error(err, "Failed to create Loki client")
		os.Exit(1)
	}
	if lokiClient.IsEnabled() {
		setupLog.Info("Loki log aggregation enabled", "address", lokiOptions.Address)
	} else {
		setupLog.Info("Loki log aggregation disabled (optional)")
	}
	...
	setupLog.Info("Context aggregator initialized", "metricsWindow", metricsWindow.String())

	// 5. LLM Client (OpenAI)
	llmOptions := llm.DefaultClientOptions()
	llmClient, err := llm.NewClient(llmOptions)
	if err != nil {
		setupLog.Error(err, "Failed to create LLM client")
		os.Exit(1)
	}
